...
def pivot_months_pandas(data):
    ...
    # add a column 'month'
    data_monthly = data.assign(month=data['date'].apply(date_to_month))

    #groupby and aggregation
    group_values_monthly = data_monthly.groupby(['name','month'])['precipitation'].agg('sum').reset_index()
    pivot_values_monthly = group_values_monthly.pivot(index='name', columns='month', values='precipitation')
    # print(pivot_values_monthly)

    group_count_monthly = data_monthly.groupby(['name','month'])['precipitation'].agg('count').reset_index()
    pivot_count_monthly = group_count_monthly.pivot(index='name', columns='month', values='precipitation')
    # print(pivot_count_monthly)
       
    return pivot_values_monthly, pivot_count_monthly
